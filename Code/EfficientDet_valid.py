...
marking=pd.read_csv('augmented_train.csv')
...
class Fitter:
    ...
    def fit(self, train_loader, validation_loader):
        for e in range(self.config.n_epochs):
            ...
            t = time.time()
            summary_loss, summary_prec = self.validation(validation_loader)

            self.log(
                f'[RESULT]: Val. Epoch: {self.epoch}, mAP: {summary_prec.avg:.5f}, summary_loss: {summary_loss.avg:.5f}, time: {(time.time() - t):.5f}')
            if summary_prec.avg > self.best_summary_prec:
                self.best_summary_prec = summary_prec.avg
                self.model.eval()
